...
async def list_entries(
    ...
    # and description from the response. All other listings (soirées,
    # mensuelles, workshops, featured, etc.) return the full payload so
    # cards keep their cover images.
    is_festival_list = (type == "festival")
    if is_festival_list:
        items = await db.entries.find(
            query, {"_id": 0, "cover_photo": 0, "description": 0}
        ).to_list(2000)
    else:
